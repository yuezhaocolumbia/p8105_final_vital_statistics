...
nyc_demo_gender = readxl::read_xlsx("./data/t_sf1_p10_nyc.xlsx", range = "G5:J29") %>%
  mutate(gender = tolower(gender)) %>%
  rename("borough_population" = "number") %>%
  spread(key = year, value = borough_population) %>%
  rename("2000_population" = "2000", "2010_population" = "2010") 

  

gender_death_data2 = merge(gender_death_data, nyc_demo_gender, by = c("gender", "borough")) 

gender_death_data2 = gender_death_data2 %>%
  mutate(population = ifelse(year < 2010, gender_death_data2$`2000_population`, gender_death_data2$`2010_population`))  %>%
  select(borough, cd_number, gender, year, cause_of_death, number, population) %>%
  rename("population_borough" = "population") %>%
  arrange(year, borough, cd_number)
...
age_death_data = age_death_data %>%
  mutate(cause_of_death = gsub("c", "x", age_death_data$cause_of_death))
...
race_death_data = my_tidy_data %>%
  ...
  mutate(
    cause_of_death = 
      substr(cause_of_death, start = 1, stop = nchar(race_death_data2$cause_of_death)-1)
                  ) %>%
  mutate(cause_of_death = gsub("c", "x", race_death_data2$cause_of_death))
```
