...
gender_death_data = my_tidy_data %>%
   ...
   mutate(gender = ifelse(str_detect(gender_death_data$cause_of_death, "female"), "female", "male"))%>%     mutate(cause_of_death = 
      substr(gender_death_data$cause_of_death,1,str_locate(gender_death_data$cause_of_death, "[0-9][a-zA-Z]")[1,])
)   %>%
  mutate(cause_of_death = gsub("c","x", gender_death_data$cause_of_death))

gender_death_data = gender_death_data %>%
  mutate(cause_of_death = gsub("m","", gender_death_data$cause_of_death))
gender_death_data = gender_death_data %>%
  mutate(cause_of_death = gsub("f","", gender_death_data$cause_of_death))


  
  

  merge(gender_death_data, nyc_demo_gender, by = c("gender", "borough"))
...
Now I added specific cause od death to the dataset.
```{r}
