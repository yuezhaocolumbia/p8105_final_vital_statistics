...
filter(year %in% c("2000", "2006", "2010", "2014")) %>%
  group_by(year) %>%
  top_n(n = 10, wt = motality_rate) 
  
lead_cause = merge(year_cs_death_ny, total_death_data) %>% 
  group_by(year, cause_of_death) %>%
  summarise(cs_death = sum(number, na.rm = TRUE)) 

lead_cause %>%
  filter(year == 2000) %>%
  ggplot(aes(reorder(x = cause_of_death, cs_death), y = cs_death)) +
...
lead_cause %>%
 ...
 lead_cause %>%
   filter(year == 2010) %>%
  ggplot(aes(reorder(x = cause_of_death, cs_death), y = cs_death)) +
  geom_bar(stat = "identity", fill = "bisque1") + 
  theme_bw() +
   theme(axis.text.x = element_text(angle=90, vjust=0.6)) +
   labs(x = "Cause of death", 
         y = "Crude mortality rate", 
         title = "leading cause of death in 2008") +
  coord_flip()
